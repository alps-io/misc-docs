...
----
In the above HTML5 example, the Menu section shows the use of the +login-link+ and +create-link+ ALPS descriptors applied to +HTML.A+ elements that use the associated +descriptor.name+ value for the HTML +class+ value. Both these HTML anchors have the RECOMMENDED HTML5 Custom Data Attribute of +data-alpsid+.
